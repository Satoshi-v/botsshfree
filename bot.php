...
env_msg="╔═════════•⊱✦⊰•═════════╗\n"
        ...
        env_msg+="[<b>02</b>] /info = Muestra informaciones\n"
        env_msg+="[<b>03</b>] /ayuda = Muestra info de opciones\n\n"
        env_msg+="⚠️ <i>Opciones Disponibles</i>\n\n"
        env_msg+="• <u>CREAR USUARIO</u> = Crear usuario ssh\n\n"
        env_msg+="• <u>CREAR TEST</u> = Crear test ssh\n\n"
        env_msg+="• <u>REMOVER</u> = Borra usuario ssh\n\n"
        env_msg+="• <u>INFO USUARIOS</u> = Informacion de los usuarios\n\n"
        env_msg+="• <u>USUARIOS ONLINE</u> = Muestra Usuarios online\n\n"
        env_msg+="• <u>INFO VPS</u> = Muestra info del servidor\n\n"
        env_msg+="• <u>CAMBIAR CONTRASEÑA</u> = Alterar contraseña ssh\n\n"
        env_msg+="• <u>ALTERAR LIMITE</u> = Altera limite ssh\n\n"
        env_msg+="• <u>ALTERAR FECHA</u> = Altera dato de fecha ssh\n\n"
        env_msg+="• <u>BORRAR EXPIRADOS</u> = Remueve ssh expirados\n\n"
        env_msg+="• <u>BACKUP</u> = Crea Backup ssh y reventas\n\n"
        env_msg+="• <u>OPTIMIZAR</u> = Limpa cache - ram\n\n"
        env_msg+="• <u>SPEEDTEST</u> = Test de conexion\n\n"
        env_msg+="• <u>ARCHIVOS</u> = Hospedar Archivos\n\n"
        env_msg+="• <u>REVENTAS</u> = Gerenciar Reventas\n\n"
        env_msg+="• <u>AUTOBACKUP</u> = Backup automatico\n\n"
        env_msg+="• <u>INFORME</u> = Informaciones sobre reventas\n\n"
        env_msg+="• <u>AYUDA</u> = Muestra info de opciones\n\n"
...
alterar_data_user() {
    ...
    [[ "$(echo -e "$inputdate" | sed -e 's/[^/]//ig')" != '//' ]] && {
        ...
        [ $today -ge $timemachine ] && {
            verify='1'
            ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                --text "$(echo -e "❌ Error! Dato invalido")" \
                --parse_mode html
            _erro='1'
            return 0
        }
...
fun_download() {
    ...
    ShellBot.sendDocument --chat_id ${message_from_id[$id]} \
        ...
        --caption "$(echo -e "✅ CREADO CON ÉXITO ✅\n\n👤 User: <code>$(awk -F " " '/Nome/ {print $2}' $_file2)</code>\n🔑 Pass: <code>$(awk -F " " '/Senha/ {print $2}' $_file2)</code>\n📱 Dispositivos: $(awk -F " " '/Limite/ {print $2}' $_file2)\n⏱️ Expira: $(awk -F " " '/Validade/ {print $2}' $_file2)")" \
        --parse_mode html
...
relatorio_rev() {
    if [[ "${callback_query_from_id[$id]}" = "$id_admin" ]]; then
        ...
        msg+="<b>Sub-Reventas:</b> $_cont_sub\n\n"
        ...
        [[ $(grep -wc 'SUBREVENDA' /etc/bot/revenda/${callback_query_from_username}/${callback_query_from_username}) == '0' ]] && {
            ShellBot.answerCallbackQuery --callback_query_id ${callback_query_id[$id]} \
                --text "⚠️ NINGUN SUB REVENDEDOR ENCONTRADO !"
        ...
        msg+="<b>Sub-Reventas Suspendidas:</b> $_cont_sup\n"
        msg+="<b>User:</b> @${callback_query_from_username}\n"
        msg+="<b>ID:</b> <code>${callback_query_from_id}</code>\n"
...
msg_bem_vindo() {
    ...
    msg+="Para optener informaciones\nhaga clic o ejecute [ /ayuda ]\n\n"
    ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
        --text "$(echo -e $msg)" \
        --parse_mode html
    return 0
...
fun_list_rev() {
    ...
    if [[ "${callback_query_from_id[$id]}" == "$id_admin" ]]; then
        ...
        [[ "$(grep -wc "SUBREVENDA" $_patch/${callback_query_from_username}/${callback_query_from_username})" != '0' ]] && {
            while read _listsub; do
                _usub="$(echo $_listsub | awk '{print $2}')"
                [[ "$(grep -wc "$_usub" $suspensos)" == '0' ]] && _usts='ACTIVO' || _usts='SUSPENSO'
                msg1+="• @$_usub - $_usts\n"
...
ShellBot.InlineKeyboardButton --button 'menu1' --line 2 --text 'CREAR TEST' --callback_data '_criarteste'
ShellBot.InlineKeyboardButton --button 'menu1' --line 3 --text 'REMOVER' --callback_data '_deluser'
ShellBot.InlineKeyboardButton --button 'menu1' --line 4 --text 'CAMBIAR CONTRASEÑA' --callback_data '_altsenha'
ShellBot.InlineKeyboardButton --button 'menu1' --line 5 --text 'ALTERAR LIMITE' --callback_data '_altlimite'
ShellBot.InlineKeyboardButton --button 'menu1' --line 6 --text 'ALTERAR FECHA' --callback_data '_altdata'
...
ShellBot.InlineKeyboardButton --button 'menu1' --line 9 --text 'BORRAR EXPIRADOS' --callback_data '_expirados'
...
ShellBot.InlineKeyboardButton --button 'menu1' --line 8 --text 'INFORME' --callback_data '_relatorio'
...
ShellBot.InlineKeyboardButton --button 'menu2' --line 4 --text 'CAMBIAR CONTRASEÑA' --callback_data '_altsenha2'
ShellBot.InlineKeyboardButton --button 'menu2' --line 4 --text 'BORRAR EXPIRADOS' --callback_data '_expirados2'
...
ShellBot.InlineKeyboardButton --button 'menu2' --line 6 --text 'INFORME' --callback_data '_relatorio2'
...
ShellBot.InlineKeyboardButton --button 'menu3' --line 1 --text 'CREAR TEST' --callback_data '_criarteste3'
...
ShellBot.InlineKeyboardButton --button 'menu3' --line 4 --text 'CAMBIAR CONTRASEÑA' --callback_data '_altsenha3'
ShellBot.InlineKeyboardButton --button 'menu3' --line 4 --text 'BORRAR EXPIRADOS' --callback_data '_expirados3'
...
ShellBot.InlineKeyboardButton --button 'menu4' --line 2 --text 'REMOVER REVENDEDOR' --callback_data '_delrev'
ShellBot.InlineKeyboardButton --button 'menu4' --line 3 --text 'ALTERAR CUPOS DE REVENTA' --callback_data '_limrev'
ShellBot.InlineKeyboardButton --button 'menu4' --line 4 --text 'ALTERAR DATOS DE REVENTA' --callback_data '_datrev'
ShellBot.InlineKeyboardButton --button 'menu4' --line 5 --text 'LISTA DE REVENDEDORES' --callback_data '_listrev'
ShellBot.InlineKeyboardButton --button 'menu4' --line 6 --text 'SUSPENDER REVENDEDOR' --callback_data '_susprevendas'
...
while :; do
        ...
        (
            ...
            if [[ ${message_entities_type[$id]} == bot_command ]]; then
                ...
                *)
                    ...
                    [[ "${comando[0]}" = "/hrlp" || "${comando[0]}" = "/ayuda" ]] && fun_ajuda
    ...
    [[ ${message_text[$id]} != ?(+|-)+([0-9]) ]] && {
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
            --text "$(echo -e "❌ Error ! \n\nUtilize solo numeros [Ej: 30]")" \
            --parse_mode html
        >$CAD_ARQ
        break
    }
    # Validación para revendedores (máximo 31 días)
    if [[ "${message_from_id[$id]}" != "$id_admin" ]] && [[ ${message_text[$id]} -gt 31 ]]; then
        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
            --text "$(echo -e "❌ ¡Restricción! \n\nComo revendedor, solo puedes asignar hasta 31 días de acceso.\n\nIngresa un valor entre 1 y 31.")" \
            --parse_mode html
        >$CAD_ARQ
        break
    fi
    ...
    ;;
                ...
                'Nuevo limite:')
                    ...
                    [[ "$_erro" == '1' ]] && break
                    ShellBot.sendMessage --chat_id ${message_from_id[$id]} \
                        --text "$(echo -e "=×=×=×=×=×=×=×=×=×=×=×\n<b>✅ LIMITE ALTERADO !</b> !\n=×=×=×=×=×=×=×=×=×=×=×\n\n<b>Usuario:</b> $(cat /tmp/name-l)\n<b>Nuevo Limite:</b> ${message_text[$id]}")" \
                        --parse_mode html
                    ...
                    if [[ "${update_id[$id]}" ]]; then
                        ...
                        [[ $download_file -eq 1 ]] && {
                            ...
                            ShellBot.downloadFile --file_path ${return[file_path]} --dir "/tmp/file" && {
                                ...
                                msg+="*Guardado en*: /etc/bot/archivos"
                                ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                                    --text "$(echo -e "$msg")" \
                                    --parse_mode markdown
                    ...
                    [[ "${message_from_id[$id]}" == "$id_admin" ]] && {
                        ...
                        [[ $(grep -w ${message_from_username} $ativos | awk '{print $NF}') == 'revenda' ]] && {
                            [[ "$(grep -wc "$_VAR1" /etc/bot/revenda/${message_from_username}/${message_from_username})" != '0' ]] && {
                                ...
                                ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                                    --text "$(echo -e "❌ El SubRevendedor ${message_text[$id]} no existe")" \
                                    --parse_mode html
                                break
                            }
                        }
